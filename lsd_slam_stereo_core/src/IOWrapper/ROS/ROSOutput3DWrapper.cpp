...
* along with LSD-SLAM-STEREO. If not, see <http://www.gnu.org/licenses/>.
*/
...
ROSOutput3DWrapper::ROSOutput3DWrapper(int width, int height)
{
	...
	liveframe_channel = nh_.resolveName("lsd_slam_stereo/liveframes");
	liveframe_publisher = nh_.advertise<lsd_slam_stereo_viewer::keyframeMsg>(liveframe_channel,1);

	keyframe_channel = nh_.resolveName("lsd_slam_stereo/keyframes");
	keyframe_publisher = nh_.advertise<lsd_slam_stereo_viewer::keyframeMsg>(keyframe_channel,1);

	graph_channel = nh_.resolveName("lsd_slam_stereo/graph");
	graph_publisher = nh_.advertise<lsd_slam_stereo_viewer::keyframeGraphMsg>(graph_channel,1);

	debugInfo_channel = nh_.resolveName("lsd_slam_stereo/debug");
	debugInfo_publisher = nh_.advertise<std_msgs::Float32MultiArray>(debugInfo_channel,1);

	pose_channel = nh_.resolveName("lsd_slam_stereo/pose");
...
{
	lsd_slam_stereo_viewer::keyframeMsg fMsg;
...
{
	lsd_slam_stereo_viewer::keyframeGraphMsg gMsg;
